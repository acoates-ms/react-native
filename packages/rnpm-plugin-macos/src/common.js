...
function getLatestVersion() {
  return new Promise(function (resolve, reject) {
    npm.packages.release('react-native-macos', 'latest', (err, releases) => {
      if (err) {
        reject(err);
      } else if (!releases || releases.length === 0) {
        reject(new Error('Could not find react-native-macos@latest.'));
      } else {
...
function getMatchingVersion(version, tag, ignoreStable) {
  console.log(`Checking for react-native-macos version matching ${version}...`);
  return new Promise(function (resolve, reject) {
    npm.packages.range('react-native-macos', version, (err, release) => {
      if (err || !release) {
        return getLatestVersion()
          .then(latestVersion => {
            reject(new Error(`Could not find react-native-macos@${version}. ` +
              `Latest version of react-native-macos is ${latestVersion}, try switching to ` +
              `react-native@${semver.major(latestVersion)}.${semver.minor(latestVersion)}.*.`));
            }).catch(error => reject(new Error(`Could not find react-native-macos@${version}.`)));
      }
      ...
      if (!isVersionMatch(matchedVersion, version, tag) && (ignoreStable || isPrerelease)) {
        ...
        if (candidates.length === 0) {
          ...
          if (tagMatches.length === 0) {
            reject(new Error(`Could not find react-native-macos@${version}-${tag}.*.`));
          } else {
            const latestVersion = tagMatches[0];
            reject(new Error(`Could not find react-native-macos@${version}-${tag}.*. ` +
              `Latest version of react-native-macos for tag '${tag}' is ${latestVersion}, try switching to ` +
              `react-native@${semver.major(latestVersion)}.${semver.minor(latestVersion)}.*.`));
          }
        }
...
const getInstallPackage = function (version, tag, useStable) {
  const packageToInstall = 'react-native-macos';
